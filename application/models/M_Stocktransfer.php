...
class m_stocktransfer extends CI_Model
{
    ...
    {
        $this->db->select('tst.*, MD5(tst.id) md5_id, two.name warehouse_origin,twd.name warehouse_destination, tu.name name_creator, tu.id id_user_created');
        $this->db->select('COUNT(tsd.id) c_material, ti.id id_inbound');
    ...
    public function get_stocktransfer_detail($where = array(), $order = '')
    {
        ...
        $this->db->select('ti.po_no, tip.lot_number');
