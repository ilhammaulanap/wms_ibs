                  ...
                  <div class="col-md-12">
                      <div class="card card-primary">
                          ...
                          <div class="card-body">
                              <div class="row">
                                  <div class="col-md-5">
                                      <button class="btn btn-primary" id="btn_add_supplier">
                  ...
                  <div class="modal-content">
                      ...
                      <div class="modal-body">
                          <div class="form-group">
                              ...
                              <input type="text" name="name" id="name" class="form-control" title="Only alphanumeric characters." pattern="^[a-zA-Z0-9\s]+$" maxlength="100" minlength="5" placeholder="Supplier Name" required>
                          </div>
                      </div>
  ...
  <script>
      $(document).ready(function() {
          ...
          var table = $('#table_supplier').DataTable({
              ajax: {
                  url: "<?= site_url('supplier/get_ajax_data_supplier') ?>",
                  type: 'POST'
              },
              order: [],
              "responsive": true,
          ...
          $('#btn_add_supplier').on('click', function() {
              $('#modal_supplier .modal-title').html('Add New Supplier');
              if (mode == 'edit') {
                  $('#id').val('').change();
                  $('#name').val('').change();
              }
          ...
          $('#table_supplier').on('click', '.btn-edit', function() {
              mode = 'edit';
              ...
              $('#modal_supplier .modal-title').html('Edit Data Supplier');
              $('#modal_supplier').modal();
          })

          $('#table_supplier').on('click', '.btn-delete', function() {
              var row = table.row($(this).parent().parent()).data();
              if (confirm('Are you sure want to delete this data ?')) {
                  $.ajax({
                      url: "<?= site_url('supplier/delete_data') ?>",
                      data: {
                          id: row['id']
                      },
                      type: 'POST',
          ...
          $('#form_supplier').on('submit', function(e) {
              ...
              if (confirm('Are you sure want to submit this data ?')) {
                  ...
                  $.ajax({
                      url: "<?= site_url('supplier/save_data') ?>",
                      data: formData,
                      processData: false,
                      contentType: false,
                      async: false,
          ...
          $('#btn_sample').on('click', function() {
              window.open("<?= site_url('supplier/export_sample_data') ?>");
          })
          ...
          $('#form_upload').on('submit', function(e) {
              ...
              $.ajax({
                  url: "<?= site_url('supplier/save_upload_data') ?>",
                  data: formData,
                  processData: false,
                  contentType: false,
                  async: false,
