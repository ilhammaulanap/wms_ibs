...
class Supplier extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->template->load('layout/v_layout', 'supplier/v_supplier', $data);
    }

    public function get_ajax_data_supplier()
    {
        $data = $this->m_app->select_global('tb_supplier', array('deletedate IS NULL' => NULL))->result_array();
        $final['draw'] = 1;
    ...
    public function save_data()
    {
        ...
        if ($id == '' || $id == NULL) {
            $id = $this->m_app->insert_global('tb_supplier', $data);
            if ($id > 0) {
            ...
            $update = $this->m_app->update_global('tb_supplier', array('id' => $id), $data);
            if ($update >= 0) {
                echo json_encode(array(
                    'code' => 200,
                    'message' => 'Data Berhasil Disimpan',
    ...
    public function get_ajax_data()
    {
        ...
        $response       = $this->m_supplier->get_data_supplier($searchTerm);
    ...
    public function export_sample_data()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', 'Import Data Supplier');
        //$spreadsheet->setActiveSheetIndex(0)->mergeCells('A1');

        //Set Judul Kolom
        $spreadsheet->setActiveSheetIndex(0)
            ->setCellValue('A2', 'Nama Supplier');
        ...
        header('Content-Disposition: attachment;filename="template_data_supplier.xlsx"');
            ...
            foreach ($sheet as $row) {
                if ($numrow > 2) {
                    ...
                    if ($row['A'] == '') {
                        ...
                        $where['name'] = $row['A'];
                        $select = $this->m_app->select_global('tb_supplier', $where);
                        ...
                        $insert = $this->m_app->insert_global('tb_supplier', $datau);
                        if ($insert > 0) {
