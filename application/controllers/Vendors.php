...
class Vendors extends CI_Controller
{
    ...
    {
        $data = [
            'title' => 'Manage Data Vendor'
        ];

        $this->template->load('layout/v_layout', 'vendors/v_vendors', $data);
    }

    public function get_ajax_data_vendor()
    {
        $data = $this->m_app->select_global('tb_vendor', array('deletedate IS NULL' => NULL))->result_array();
        $final['draw'] = 1;
    ...
    public function save_data()
    {
        ...
        if ($id == '' || $id == NULL) {
            $id = $this->m_app->insert_global('tb_vendor', $data);
            if ($id > 0) {
            ...
            $update = $this->m_app->update_global('tb_vendor', array('id' => $id), $data);
            if ($update >= 0) {
                echo json_encode(array(
                    'code' => 200,
                    'message' => 'Data Berhasil Disimpan',
    ...
    public function get_ajax_data()
    {
        ...
        $response       = $this->m_vendor->get_array_id($searchTerm);
    ...
    public function export_sample_data()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)->setCellValue('A1', 'Import Data Vendor');
        //$spreadsheet->setActiveSheetIndex(0)->mergeCells('A1');

        //Set Judul Kolom
        $spreadsheet->setActiveSheetIndex(0)
            ->setCellValue('A2', 'Nama Vendor');
        ...
        header('Content-Disposition: attachment;filename="template_data_vendor.xlsx"');
            ...
            foreach ($sheet as $row) {
                if ($numrow > 2) {
                    ...
                    if ($row['A'] == '') {
                        ...
                        $select = $this->m_app->select_global('tb_vendor', $where);
                        ...
                        $insert = $this->m_app->insert_global('tb_vendor', $datau);
                        if ($insert > 0) {
